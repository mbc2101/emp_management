...
// Vérifier si l'ID du périphérique est fourni
if (!isset($_GET['devicetypeid'])) {
    echo "No device type ID provided.";
    exit;
}

// Récupérer l'ID du périphérique
$deviceId = $_GET['devicetypeid'];

// Récupérer les informations du périphérique à modifier
$query = $pdo->prepare('SELECT * FROM devicetypes WHERE devicetypeid = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['form_submit'])) {
    ...
    // Mettre à jour le type de périphérique dans la base de données
    $updateStmt = $pdo->prepare('
        UPDATE devicetypes 
        SET brand = ?, model = ?, size = ?, memory = ?, os = ?, sim = ?, resolution = ? 
        WHERE devicetypeid = ?
    ');
    $updateStmt->execute([$brand, $model, $size, $memory, $os, $sim, $resolution, $deviceId]);
    ...
    <div class="container form-container mx-auto">
        ...
        <form id="deviceForm" method="POST" action="">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="brand" name="brand" value="<?= htmlspecialchars($device['brand']); ?>" required autofocus>
            </div>
            <div class="form-group">
                <label for="model">Model</label>
                <input type="text" class="form-control" id="model" name="model" value="<?= htmlspecialchars($device['model']); ?>" required>
            </div>
            <div class="form-group">
                <label for="size">Size</label>
                <input type="text" class="form-control" id="size" name="size" value="<?= htmlspecialchars($device['size']); ?>" required>
            </div>
            <div class="form-group">
                <label for="memory">Memory</label>
                <input type="text" class="form-control" id="memory" name="memory" value="<?= htmlspecialchars($device['memory']); ?>" required>
            </div>
            <div class="form-group">
                <label for="os">Operating System</label>
                <input type="text" class="form-control" id="os" name="os" value="<?= htmlspecialchars($device['os']); ?>" required>
            </div>
            <div class="form-group">
                <label for="sim">SIM</label>
                <div>
                    <input type="radio" id="simWith" name="sim" value="1" <?= $device['sim'] == '1' ? 'checked' : ''; ?> required>
                    <label for="simWith">Device with SIM</label>
                </div>
                <div>
                    <input type="radio" id="simWithout" name="sim" value="0" <?= $device['sim'] == '0' ? 'checked' : ''; ?> required>
                    <label for="simWithout">Device without SIM</label>
                </div>
            </div>
            <div class="form-group">
                <label for="resolution">Resolution</label>
                <input type="text" class="form-control" id="resolution" name="resolution" value="<?= htmlspecialchars($device['resolution']); ?>" required>
            </div>
