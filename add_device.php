...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['serialnumber'])) {
    ...
    try {
        // Insert new device
        $stmt = $pdo->prepare('INSERT INTO devices (serialnumber, macaddress, devicetypeid, statusid) VALUES (?, ?, ?, ?)');
        $stmt->execute([$serialnumber, $macaddress, $devicetypeid, $statusid]);
        ...
        echo "Error inserting device: " . $e->getMessage();
...
<?php include 'navbar.php'; ?> <!-- Inclure la barre de navigation -->
    ...
    <div class="container">
        <div class="form-container mx-auto">
            <h2 class="text-center"><i class="fas fa-plus"></i> Add New Device</h2>
            <form id="deviceForm" method="POST" onsubmit="event.preventDefault(); validateForm();">
                <!-- Device Details -->
                <div class="mb-3">
                    <label for="serialnumber" class="form-label">Serial Number</label>
                    <input type="text" class="form-control" id="serialnumber" name="serialnumber" required autofocus>
                </div>
                <div class="mb-3">
                    <label for="macaddress" class="form-label">MAC Address</label>
                    <input type="text" class="form-control" id="macaddress" name="macaddress" required maxlength="12">
