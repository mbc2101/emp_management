...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['planname'])) {
    ...
    try {
        // Insert new plan
        $stmt = $pdo->prepare('INSERT INTO plansdefinitions (planname, price, maxmessages, maxphotos, officialplan, enabled) VALUES (?, ?, ?, ?, ?, ?)');
        $stmt->execute([$planname, $price, $maxmessages, $maxphotos, $officialplan, $enabled]);

        // Redirect back to the plan list
        header('Location: plans.php');
        exit;
    } catch (PDOException $e) {
        echo "Error inserting plan: " . $e->getMessage();
    }
}
?>
        ...
        <div class="form-container mx-auto">
            ...
            <form id="planForm" method="POST" onsubmit="event.preventDefault(); showPreview();">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="planname" name="planname" required autofocus>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="maxmessages" name="maxmessages" required>
                </div>
                <div class="mb-3">
                    <label for="maxphotos" class="form-label">Max Photos</label>
                    <input type="number" class="form-control" id="maxphotos" name="maxphotos" required>
                </div>
                <div class="form-check mb-3">
                    <input type="checkbox" class="form-check-input" id="officialplan" name="officialplan">
