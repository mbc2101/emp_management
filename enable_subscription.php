...
if (isset($_GET['subscriptionid'])) {
    ...
    $sql = 'UPDATE subscriptions SET enabled = 1 WHERE subscriptionid = :subscriptionid';
    $stmt = $pdo->prepare($sql);
    
    // Execute the statement
    if ($stmt->execute(['subscriptionid' => $subscriptionId])) {
        // Redirect back to subscriptions list with a success message
        header('Location: subscriptions.php?message=Subscription enabled successfully.');
        exit;
    } else {
        // Redirect back with an error message
        header('Location: subscriptions.php?error=Could not enable the subscription.');
        exit;
    }
} else {
